#!/bin/bash
# ============================================================
# flash_secure_v1.sh - Secure Vehicle 1 Deployment (SDV001)
# ============================================================
# This script:
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Secure Vehicle 1 Deployment Script (SDV001)          ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════╝${NC}\n"
...
case $choice in
    1)
        ...
        if [ ! -f "scripts/setup_credentials_v1.cpp" ]; then
            echo -e "${RED}❌ Error: scripts/setup_credentials_v1.cpp not found${NC}"
            echo -e "${YELLOW}💡 Create this file first with the credentials for Vehicle 1${NC}"
            exit 1
        fi
        ...
        flash_and_wait \
            ...
            "scripts/setup_credentials_v1.cpp" \
            "5"
...
if [ ! -f "backup/main_1_secure.cpp" ]; then
    echo -e "${YELLOW}⚠️  backup/main_1_secure.cpp not found${NC}"
    echo -e "${YELLOW}    Using backup/main_1.cpp instead${NC}\n"
    MAIN_SOURCE="backup/main_1.cpp"
else
    MAIN_SOURCE="backup/main_1_secure.cpp"
fi
...
echo ""
echo -e "${GREEN}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  ✅ Vehicle 1 (SDV001) Deployed Successfully!         ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════════╝${NC}\n"
